...
/*
CHANGELOG (this file)
- Added 5 filters (ADXTrend, PSARTrend, MACDTrend, StochSignal, VolumeAboveMA) to A/B/C/D filter banks for S1 & S2
- Extended filter banks to E/F for S1 & S2
- Added LossLockoutTrades (per-dir per S1/S2) and PriceImproveATR gates for entries
- Preserved trailing placeholders from v3.8026
*/
// ===== ENUMS =====
...
struct StrategyFlags {
  ...
  bool checkRSIAtConfirmBuy; bool checkRSIAtConfirmSell; // Flags for immediate RSI check
  int crossShift; // To track the shift where cross happened
...
int    skipBuyCountS1 = 0, skipSellCountS1 = 0, skipBuyCountS2 = 0, skipSellCountS2 = 0; // NEW: trade-skip counters
// ===== SIGNAL ENUM =====
...
double CalcStopLoss(StopLossEnum st,SLTypeEnum mode,double entry,bool isBuy,double val, int atr_handle = INVALID_HANDLE){
  ...
  if(st==Fixed_Stop || st==Custom_Trailing || (st==Trailing_Stop && UseInitialSLForTrailing1)){
    ...
    double atr = GetATR(atr_handle, 1); // Use ATR of previous bar
    if(atr < 0) return 0;
...
// NEW: ATRFilter check (blocks if ATR > threshold, independent of isBuy)
bool CheckFilter_ATR(int h_atr, double threshold){
  ...
  double atr = GetATR(h_atr, 1); // Use shift=1 for closed bar
                   ...
                   int h_adx, int h_psar, int h_macd, int h_stoch)
{
  ...
  if(f==HeikinFilter) {
    int shift = HeikinFilter_UseClosedBar ? 1 : 0;
    return CheckFilter_Heikin(h_ha, isBuy, shift);
  }
                    ...
                    int hD_ema_trend, int hD_rsi_fast, int hD_rsi_slow, int hD_ema_fast, int hD_ema_slow, int hD_ha, int hD_atr, int hD_adx, int hD_psar, int hD_macd, int hD_stoch)
{
  ...
  if(logic==LOGIC_AND) return (a && b && c && d);
  else return (a || b || c || d); // LOGIC_OR
}
// ===== ENTRIES =====
...
// Updated RSICrossHeikinV2 Entry
...
int OnInit(){
  ...
  // HeikinSAR strategy PSAR handles (only if selected)
...
bool PassEntryGates(bool isBuy, bool forS1, ENUM_TIMEFRAMES tf){
  // 1) Skip N valid entries after SL
  if(LossLockoutTrades>0){
    if(forS1){ if(isBuy && skipBuyCountS1>0){ skipBuyCountS1--; return false; } if(!isBuy && skipSellCountS1>0){ skipSellCountS1--; return false; } }
    else { if(isBuy && skipBuyCountS2>0){ skipBuyCountS2--; return false; } if(!isBuy && skipSellCountS2>0){ skipSellCountS2--; return false; } }
  }
  // 2) Time-based lockout and price improvement only when target filters present
  bool hasTarget = forS1 ? HasTargetFilters_S1() : HasTargetFilters_S2();
  if(hasTarget){
    if(LossLockoutBars>0){ int curBar = Bars(_Symbol, tf); if(forS1){ if(isBuy && lockoutBuyUntilBarS1>0 && curBar < lockoutBuyUntilBarS1) return false; if(!isBuy && lockoutSellUntilBarS1>0 && curBar < lockoutSellUntilBarS1) return false; } else { if(isBuy && lockoutBuyUntilBarS2>0 && curBar < lockoutBuyUntilBarS2) return false; if(!isBuy && lockoutSellUntilBarS2>0 && curBar < lockoutSellUntilBarS2) return false; } }
    if(PriceImproveATR>0){ int ah = forS1 ? atr_handle1 : atr_handle2; if(ah==INVALID_HANDLE) ah = iATR(_Symbol, tf, ATR_Period); double atr = GetATR(ah, 1); if(atr>0){ double minDelta = PriceImproveATR * atr; double cur = isBuy ? SymbolInfoDouble(_Symbol, SYMBOL_ASK) : SymbolInfoDouble(_Symbol, SYMBOL_BID); if(forS1){ if(isBuy && lastEntryPriceBuyS1>0 && cur < lastEntryPriceBuyS1 + minDelta) return false; if(!isBuy && lastEntryPriceSellS1>0 && cur > lastEntryPriceSellS1 - minDelta) return false; } else { if(isBuy && lastEntryPriceBuyS2>0 && cur < lastEntryPriceBuyS2 + minDelta) return false; if(!isBuy && lastEntryPriceSellS2>0 && cur > lastEntryPriceSellS2 - minDelta) return false; } } }
  }
  return true;
}
// ===== MAIN =====
void OnTick(){
  ...
  if(hasPosition && ForceCloseNow(hour,min)){ trade.PositionClose(_Symbol); ZeroMemory(strat1flags); ZeroMemory(strat2flags); return; }
  ...
  } else if(!allowEntries && !hasPosition) { ZeroMemory(strat1flags); ZeroMemory(strat2flags); }

  // ===== EXITS (strategy-based optional) =====
  if(PositionSelect(_Symbol)){
    bool isBuy = (PositionGetInteger(POSITION_TYPE) == POSITION_TYPE_BUY);
    string usedStrat=""; string pc = PositionGetString(POSITION_COMMENT);
    int p=StringFind(pc,"STRAT="); if(p>=0){ usedStrat = StringSubstr(pc, p+6); int sp = StringFind(usedStrat, " "); int rp = StringFind(usedStrat, ")"); int cut = sp; if(cut<0 || (rp>=0 && rp<cut)) cut = rp; if(cut>0) usedStrat = StringSubstr(usedStrat, 0, cut); StringTrimLeft(usedStrat); StringTrimRight(usedStrat); }
...
// ===== TRADE EVENTS: set lockout after SL; reset on profit =====
void OnTradeTransaction(const MqlTradeTransaction& trans, const MqlTradeRequest& request, const MqlTradeResult& result)
{
  if(trans.type != TRADE_TRANSACTION_DEAL_ADD) return;
  ulong deal = trans.deal; int entry = (int)HistoryDealGetInteger(deal, DEAL_ENTRY); if(entry != DEAL_ENTRY_OUT) return;
  ...
  bool fromS1 = (StringFind(cmt," S1")>=0);
  int reason = (int)HistoryDealGetInteger(deal, DEAL_REASON);
  double profit = HistoryDealGetDouble(deal, DEAL_PROFIT);
  // Skip trades after SL
  if(LossLockoutTrades>0 && reason == DEAL_REASON_SL){ if(fromS1){ if(wasBuy) skipBuyCountS1 = LossLockoutTrades; else skipSellCountS1 = LossLockoutTrades; } else { if(wasBuy) skipBuyCountS2 = LossLockoutTrades; else skipSellCountS2 = LossLockoutTrades; } }
  // Reset counters on profit
  if(profit > 0.0){ if(fromS1){ if(wasBuy) skipBuyCountS1 = 0; else skipSellCountS1 = 0; } else { if(wasBuy) skipBuyCountS2 = 0; else skipSellCountS2 = 0; } }
  // Time-based lockout on SL
  if(LossLockoutBars>0 && reason == DEAL_REASON_SL){ ENUM_TIMEFRAMES tf = fromS1 ? Strategy1Timeframe : Strategy2Timeframe; int curBar = Bars(_Symbol, tf); int until = curBar + LossLockoutBars; if(fromS1){ if(wasBuy) lockoutBuyUntilBarS1 = until; else lockoutSellUntilBarS1 = until; } else { if(wasBuy) lockoutBuyUntilBarS2 = until; else lockoutSellUntilBarS2 = until; } }
}
...
bool FilterPass(FilterType type,bool forBuy, ENUM_TIMEFRAMES tf, bool useClosedBar){ int sh = useClosedBar? 1:0; switch(type){
  ...
  case FT_EMA_TREND:{ int per=Inp_EMA_Period_Default; int mode=Inp_EMA_Mode_Default; int price=Inp_EMA_Price_Default; int h=iMA(_Symbol,tf,per,0,mode,price); double ema[]; ArraySetAsSeries(ema,true); ArrayResize(ema,1); if(CopyBuffer(h,0,sh,1,ema)<1) return false; double cl=iClose(_Symbol,tf,sh); return forBuy? (cl>ema[0]) : (cl<ema[0]); }
  case FT_EMA_CROSS:{ int f=Inp_EMA_Fast_Default, s=Inp_EMA_Slow_Default; int mode=Inp_EMA_Mode_Default; int price=Inp_EMA_Price_Default; int hf=iMA(_Symbol,tf,f,0,mode,price); int hs=iMA(_Symbol,tf,s,0,mode,price); double af[], as[]; ArraySetAsSeries(af,true); ArraySetAsSeries(as,true); ArrayResize(af,2); ArrayResize(as,2); if(CopyBuffer(hf,0,sh,2,af)<2) return false; if(CopyBuffer(hs,0,sh,2,as)<2) return false; bool up   =(af[1]<=as[1] && af[0]>as[0]); bool down =(af[1]>=as[1] && af[0]<as[0]); return forBuy? up:down; }
  case FT_RSI_X2_CROSS:{ int rf=Inp_RSI_Fast_Default, rs=Inp_RSI_Slow_Default; int hf=iRSI(_Symbol,tf,rf,Inp_RSI_AppliedPrice); int hs=iRSI(_Symbol,tf,rs,Inp_RSI_AppliedPrice); double af[],as2[]; ArraySetAsSeries(af,true); ArraySetAsSeries(as2,true); ArrayResize(af,2); ArrayResize(as2,2); if(CopyBuffer(hf,0,sh,2,af)<2) return false; if(CopyBuffer(hs,0,sh,2,as2)<2) return false; bool up=(af[1]<=as2[1] && af[0]>as2[0]); bool down=(af[1]>=as2[1] && af[0]<as2[0]); return forBuy? up:down; }
  case FT_RSI_BOUNDS_CROSS:{ int hr=iRSI(_Symbol,tf,Inp_RSI_Period,Inp_RSI_AppliedPrice); double r[]; ArraySetAsSeries(r,true); ArrayResize(r,2); if(CopyBuffer(hr,0,sh,2,r)<2) return false; bool buy=(r[1]<Inp_RSI_Low && r[0]>=Inp_RSI_Low); bool sell=(r[1]>Inp_RSI_High && r[0]<=Inp_RSI_High); return forBuy? buy:sell; }
  case FT_ADX_TREND:{ int ha=iADX(_Symbol,tf,Inp_ADX_Period_Default); double adx[],pdi[],mdi[]; ArraySetAsSeries(adx,true); ArraySetAsSeries(pdi,true); ArraySetAsSeries(mdi,true); ArrayResize(adx,1); ArrayResize(pdi,1); ArrayResize(mdi,1);
  ...
  case FT_PSAR_TREND:{ int h=iSAR(_Symbol,tf,Inp_PSAR_Step_Default,Inp_PSAR_Max_Default); double ps[]; ArraySetAsSeries(ps,true); ArrayResize(ps,1); if(CopyBuffer(h,0,sh,1,ps)<1) return false; double cl=iClose(_Symbol,tf,sh); return forBuy? (cl>ps[0]) : (cl<ps[0]); }
  case FT_HEIKIN_FILTER:{ bool bull=HeikinBull(tf,sh); return forBuy? bull:(!bull); }
  case FT_ATR_GATE:{ int h=iATR(_Symbol,tf,Inp_ATR_Period_Default); double a[]; ArraySetAsSeries(a,true); ArrayResize(a,1); if(CopyBuffer(h,0,sh,1,a)<1) return false; return (a[0]>=Inp_ATR_Threshold_Default); }
  case FT_VOLUME:{
    int vshift = (useClosedBar?1:0);
    long v = (long)iVolume(_Symbol,tf,vshift);
    double vol = (double)v;
    if(Inp_VolumeMode_Default==VM_ABS_THRESHOLD){ return (vol>=Inp_VolumeThreshold_Default); }
    else { // MA_RATIO
      double sma=0.0; int n=Inp_VolumeMAPeriod_Default; int got=0; for(int i=vshift; i<vshift+n; ++i){ long vi = (long)iVolume(_Symbol,tf,i); if(vi<=0) break; sma+=(double)vi; got++; }
...
// ========================= RSI CROSS HELPERS =========================
bool RSICrossUpThreshold(ENUM_TIMEFRAMES tf, EvalMode em, double thr){ int sh = (em==EM_CLOSED_BAR)?1:0; int hp=iRSI(_Symbol,tf,Inp_RSI_Period,Inp_RSI_AppliedPrice); double r[]; ArraySetAsSeries(r,true); ArrayResize(r,2); if(CopyBuffer(hp,0,sh,2,r)<2) return false; return (r[1]<thr && r[0]>=thr); }
bool RSICrossDownThreshold(ENUM_TIMEFRAMES tf, EvalMode em, double thr){ int sh = (em==EM_CLOSED_BAR)?1:0; int hp=iRSI(_Symbol,tf,Inp_RSI_Period,Inp_RSI_AppliedPrice); double r[]; ArraySetAsSeries(r,true); ArrayResize(r,2); if(CopyBuffer(hp,0,sh,2,r)<2) return false; return (r[1]>thr && r[0]<=thr); }
...
double ComputeATRPoints(ENUM_TIMEFRAMES tf,int period,double mult){ int h=iATR(_Symbol,tf,period); double a[]; ArraySetAsSeries(a,true); ArrayResize(a,1); if(CopyBuffer(h,0,1,1,a)<1) return 0.0; return mult*(a[0]/_Point); }
...
void ComputeStopSetSL(bool isBuy, StopSetType type, UnitMode mode, ENUM_TIMEFRAMES tf, bool useClosedBar,
  ...
  switch(type){
    ...
    case SS_PSAR:{ int h=iSAR(_Symbol,tf,psarStep,psarMax); double ps[]; ArraySetAsSeries(ps,true); ArrayResize(ps,1); if(CopyBuffer(h,0,sh,1,ps)>0){ slOut = ps[0]; } break; }
...
double CalcLotsByRisk(double slPoints){ // slPoints in points
  ...
  double lots = (slPoints>0.0? riskValue / (slPoints*pointValuePerLot) : Inp_FixedLot);
...
bool DoEntry(bool isBuy){ if(!SpreadOK()) return false; if(!InTimeWindowEntries()) return false; if(!CanOpenMore()) return false;
  ...
  if(AccountInfoDouble(ACCOUNT_MARGIN_FREE) <= AccountInfoDouble(ACCOUNT_EQUITY) * (Inp_MinFreeMarginPct/100.0)) return false;
...
void ManageOpenPositions(){ // iterate positions for this symbol
  for(int i=PositionsTotal()-1;i>=0;--i){ if(!PositionSelectByIndex(i)) continue; string sym=PositionGetString(POSITION_SYMBOL); if(sym!=_Symbol) continue; long type=PositionGetInteger(POSITION_TYPE); bool isBuy=(type==POSITION_TYPE_BUY); double price=PositionGetDouble(POSITION_PRICE_OPEN); double sl=PositionGetDouble(POSITION_SL); double tp=PositionGetDouble(POSITION_TP);
    ...
    // Breakeven (use set 1 if configured else set 2)
    bool hasBE1 = (Inp_StopSet1_Type==SS_BREAKEVEN);
    bool hasBE2 = (Inp_StopSet2_Type==SS_BREAKEVEN);
    if(hasBE1 || hasBE2){ double cur=iClose(_Symbol, Inp_StrategyTF, 0);
      double trig = hasBE1? Inp_StopSet1_BETrigger : Inp_StopSet2_BETrigger;
      double off  = hasBE1? Inp_StopSet1_BEOffset  : Inp_StopSet2_BEOffset;
      UnitMode m  = hasBE1? Inp_StopSet1_Mode     : Inp_StopSet2_Mode;
      double triggerPoints = (m==UM_PIPS? trig*PipSize()/_Point : (price*trig/100.0)/_Point);
      if(triggerPoints>0){ if(isBuy && cur>=price+triggerPoints*_Point){ double be=(m==UM_PIPS? price + off*PipSize() : price + price*off/100.0); if(sl<be) trade.PositionModify(_Symbol,be,tp); }
        if(!isBuy && cur<=price-triggerPoints*_Point){ double be=(m==UM_PIPS? price - off*PipSize() : price - price*off/100.0); if(sl==0.0 || sl>be) trade.PositionModify(_Symbol,be,tp); } }
    }
    // Trailing (simple pips)
    bool hasTR1 = (Inp_StopSet1_Type==SS_TRAILING);
    bool hasTR2 = (Inp_StopSet2_Type==SS_TRAILING);
    if(hasTR1 || hasTR2){
      double cur=iClose(_Symbol, Inp_StrategyTF, 0);
      double tStart = hasTR1? Inp_StopSet1_TrailStart : Inp_StopSet2_TrailStart;
      double tStep  = hasTR1? Inp_StopSet1_TrailStep  : Inp_StopSet2_TrailStep;
      UnitMode m    = hasTR1? Inp_StopSet1_Mode       : Inp_StopSet2_Mode;
      double trailStartPts = (m==UM_PIPS? tStart*PipSize()/_Point : (price*tStart/100.0)/_Point);
      double trailStepPts  = (m==UM_PIPS? tStep*PipSize()/_Point  : (price*tStep/100.0)/_Point);
    ...
    bool exitBySoft    = false; // Soft handled via SL mods; here only filter-close

...
bool FilterPass(FilterType type,bool forBuy, ENUM_TIMEFRAMES tf, bool useClosedBar){ int sh = useClosedBar? 1:0; switch(type){
  ...
  case FT_ADX_TREND:{ int ha=iADX(_Symbol,tf,Inp_ADX_Period_Default); double adx[1],pdi[1],mdi[1]; ArraySetAsSeries(adx,true); ArraySetAsSeries(pdi,true); ArraySetAsSeries(mdi,true);
